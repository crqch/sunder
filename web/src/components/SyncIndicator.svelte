<script lang="ts">
  ...
  import { getUnsyncedChanges, syncAll } from '$lib/sync';
...
{:else}
  {#if !syncState.minimized && syncState.status !== 'idle'}
    <div
      ...
      class="bg-card border-border/50 fixed right-0 bottom-0 left-0 z-50 flex h-14 items-center justify-between border-t px-6 shadow-[0_-4px_10px_rgba(0,0,0,0.05)] md:left-64"
    >
